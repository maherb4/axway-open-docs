...
3. Configure the following membership fields, multiples can be added by using the '+' button provided:
   * **Organization**: Select the organization that the user belongs to. The default list includes the API Development organization only.
        ...
        {{< alert title="Note" color="primary" >}}The first organization assigned to the account will be that accounts 'Primary Org'. This is the org that the user will be logged into and is the org that the user is associated with, meaning if this org is deleted the user will be deleted too.{{< /alert >}}
